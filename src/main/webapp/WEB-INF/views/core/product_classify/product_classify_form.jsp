...
<div class="c-bar margin-top5">
  <span class="c-position"><s:message code="product.classify"/> - <s:message code="${oprt=='edit' ? 'edit' : 'create'}"/></span>
</div>
...
<table border="0" cellpadding="0" cellspacing="0" class="in-tb margin-top5">
  ...
  <tr>
    ...
    <td class="in-ctt" width="35%"><f:text name="classifyName" value="${oprt=='edit' ? (bean.classifyName) : ''}" style="width:180px;"/></td>
    <td class="in-lab" width="15%">上级分类:</td>
    <td class="in-ctt" width="35%">
    
    
    
     <select name="beforeClassifyId" id="beforeClassifyId">
        <f:option value="0" selected="0" >商品</f:option>
        <c:forEach var="productClassify" items="${productClassifyList}">
        <f:option value="${productClassify.id}" selected="${bean.beforeClassifyId}" >${productClassify.classifyName}</f:option>
        </c:forEach>
      </select>
  ...
  <tr>
    <td class="in-lab" width="15%">商品等级:</td>
...
<script >
function toVaild(){
	if($("#sourceId").val()=='1'&&$("#beforeClassifyId").val()!='0'){
		alert("上级分类选择错误");
		return false;
	}else	if($("#sourceId").val()=='2'&&$("#beforeClassifyId").val()=='0'){
		alert("上级分类选择错误");
		return false;
	}else{
		return true;
