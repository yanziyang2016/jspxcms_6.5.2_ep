...
public class OAuthLoginController {
	...
	@Value("#{properties['oauth.weixin.authorize_url']}")
	private String authorize_url;
	@Value("#{properties['oauth.weixin.appid']}")
	private String appid;
	@Value("#{properties['oauth.weixin.secret']}")
	private String secret;
	@Value("#{properties['oauth.weixin.redirect_uri']}")
	private String redirect_uri;
	@Value("#{properties['oauth.weixin.response_type']}")
	private String response_type;
	@Value("#{properties['oauth.weixin.scope']}")
	private String scope;

	@RequestMapping(value = { "/oauth/login/weixin.jspx",
			Constants.SITE_PREFIX_PATH + "/oauth/login/weixin.jspx" })
	public void loginWeixin(String fallbackUrl, HttpServletRequest request,
		...
		String url = authorize_url + "?" + "appid=" + appid + "&redirect_uri="
				...
				+ response_type + "&scope=" + scope + "&state=" + state;
	 ...
	 * @see #loginQq(HttpServletRequest, HttpServletResponse,
	 *      org.springframework.ui.Model)
	 */
	@Deprecated
	@RequestMapping(value = { "/oauth/qq_login.jspx",
			Constants.SITE_PREFIX_PATH + "/oauth/qq_login.jspx" })
	...
	public void loginWeibo(String fallbackUrl, HttpServletRequest request,
			HttpServletResponse response, org.springframework.ui.Model modelMap)
			throws IOException, WeiboException {
		String state = getState(request);
	 ...
	 * @see #loginWeibo(String, HttpServletRequest, HttpServletResponse,
	 *      org.springframework.ui.Model)
	 * 
	 */
	@Deprecated
	@RequestMapping(value = { "/oauth/weibo_login.jspx",
			Constants.SITE_PREFIX_PATH + "/oauth/weibo_login.jspx" })
	...
	private boolean validateState(String state, HttpServletRequest request) {
		String sessionState = (String) request.getSession().getAttribute(
				SESSION_OAUTH_STATE);
	...
	public String authcWeixin(String code, String state,
		...
		OauthToken ot = new OauthApi(new WeixinAccount(appid, secret))
				.getOauthToken(code);
		String ticket = ot.getAccessToken();
		logger.debug("oauth weixin access token:" + ticket);
		if (StringUtils.isBlank(ticket)) {
			return authcFail(request, response, ra,
		...
		if (StringUtils.isBlank(openid)) {
			return authcFail(request, response, ra,
					OpenIDNotFoundException.class);
		}
		OAuthToken token = new OAuthToken(OAuthToken.PROVIDER_WEIXIN, ticket,
				openid);
		User user = userShiroService.findByWeixinOpenid(openid);
		return authc(request, response, ra, token, user);
	}

	@RequestMapping(value = { "/oauth/authc/weixin/session.jspx",
			Constants.SITE_PREFIX_PATH + "/oauth/authc/weixin/session.jspx" })
	public String authcWeixinSession(HttpServletRequest request,
		...
		if (token == null) {
			return authcFail(request, response, ra,
					SessionTokenNotFoundException.class);
		}
	...
	public String authcQq(HttpServletRequest request,
			HttpServletResponse response, RedirectAttributes ra)
			throws QQConnectException, IOException {
		String ticket = (new Oauth()).getAccessTokenByRequest(request)
				.getAccessToken();
		if (StringUtils.isBlank(ticket)) {
			return authcFail(request, response, ra,
					AccessTokenNotFoundException.class);
		...
		OAuthToken token = new OAuthToken(OAuthToken.PROVIDER_QQ, ticket,
				openid);
		User user = userShiroService.findByQqOpenid(openid);
		return authc(request, response, ra, token, user);
	}

	@RequestMapping(value = { "/oauth/authc/qq/session.jspx",
			Constants.SITE_PREFIX_PATH + "/oauth/authc/qq/session.jspx" })
	public String authcQqSession(HttpServletRequest request,
			HttpServletResponse response, RedirectAttributes ra)
			throws QQConnectException, IOException {
		OAuthToken token = (OAuthToken) request.getSession().getAttribute(
				OAUTH_TOKEN_SESSION_NAME);
	...
	public String authcWeibo(HttpServletRequest request,
			HttpServletResponse response, RedirectAttributes ra)
			throws WeiboException, JSONException, IOException {
		if (validateState(request.getParameter("state"), request)) {
			return authcFail(request, response, ra, StateIllegalException.class);
		}
		String ticket = new weibo4j.Oauth().getAccessTokenByCode(
		...
		OAuthToken token = new OAuthToken(OAuthToken.PROVIDER_WEIBO, ticket,
				openid);
		User user = userShiroService.findByWeiboUid(openid);
		return authc(request, response, ra, token, user);
	}

	@RequestMapping(value = { "/oauth/authc/weibo/session.jspx",
			Constants.SITE_PREFIX_PATH + "/oauth/authc/weibo/session.jspx" })
	public String authcWeiboSession(HttpServletRequest request,
			HttpServletResponse response, RedirectAttributes ra)
			throws Exception {
		OAuthToken token = (OAuthToken) request.getSession().getAttribute(
				OAUTH_TOKEN_SESSION_NAME);
	...
	private String authc(HttpServletRequest request,
			HttpServletResponse response, RedirectAttributes ra,
		...
		PrincipalCollection principals = new SimplePrincipalCollection(
				principal, "oauth_realm_" + token.getProvider());
	...
	private String authcFail(HttpServletRequest request,
			HttpServletResponse response, RedirectAttributes ra,
		...
		ra.addAttribute(FALLBACK_URL_PARAM,
				Servlets.getParam(request, FALLBACK_URL_PARAM));
